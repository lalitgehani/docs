...
| `~`      | SQL `LIKE` pattern match   | `filter("title", "~", "%tutorial%")` |
| `IN`     | Value is in a list         | `filter("status", "IN", ["published", "featured"])` |
| `IS NULL` | Is null                   | `filter("excerpt", "IS NULL")` |
| `IS NOT NULL` | Is not null           | `filter("excerpt", "IS NOT NULL")` |
...
The `~` operator maps to SQL `LIKE`, so include `%` wildcards explicitly for contains-style matching.
...
The `~` operator uses SQL `LIKE` pattern matching:

```ts
// Matches values that contain "tutorial"
const results = await client.records
  .query("posts")
  .filter("title", "~", "%tutorial%")
  .get();
...
const postsWithoutExcerpt = await client.records
  .query("posts")
  .filter("excerpt", "IS NULL")
  .get();

// Is not null
const postsWithExcerpt = await client.records
  .query("posts")
  .filter("excerpt", "IS NOT NULL")
  .get();
```
