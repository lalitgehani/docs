...
**Single-Tenant Mode**:
The `account` field is optional. If omitted, the default singleton account is used.
...
| `filter`  | str  | null          | SnackBase filter expression |
| `cursor` | str | null | Forward cursor token |
| `cursor_before` | str | null | Backward cursor token |
| `include_count` | bool | false | Include total with cursor pagination |
...
curl -X GET "http://localhost:8000/api/v1/records/posts?sort=-created_at&skip=0&limit=10" \
  -H "Authorization: Bearer <token>"
...
# Text pattern matching. The ~ operator maps to SQL LIKE, so include % wildcards.
curl --get http://localhost:8000/api/v1/records/posts \
  --data-urlencode 'filter=name ~ "%keyword%"' \
  -H "Authorization: Bearer <token>"

# Cursor pagination
curl -X GET "http://localhost:8000/api/v1/records/posts?cursor=NEXT_CURSOR&limit=10&include_count=true" \
  -H "Authorization: Bearer <token>"
```

<Note>
  Use the `filter` parameter for record filtering. Direct field query parameters
  such as `?published=true` are rejected with `400`. Fields named like internal
  joined tables, such as `name`, are supported in filter expressions.
</Note>
...
```bash
curl -X PUT http://localhost:8000/api/v1/collections/posts/rules \
  -H "Authorization: Bearer <superadmin_token>" \
  -H "Content-Type: application/json" \
  -d '{
    "list_rule": "true",
...
- [OAuth Overview](/guides/oauth-overview)
