...
For **shared reference data** that must not be fan-out copied into every tenant
(e.g. cloud regions), use first-class [Codelists](/concepts/codelists) with
system scope and per-account overrides instead of per-account collection rows.
