...
For **shared reference data** (enums, status dictionaries), prefer first-class
[Codelists](/concepts/codelists) with system scope and per-account overrides
rather than copying rows into every tenant collection.
