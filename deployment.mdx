...
- [ ] Set strong `SNACKBASE_SECRET_KEY`, `SNACKBASE_TOKEN_SECRET` and `SNACKBASE_ENCRYPTION_KEY`
...
# Security — all three are required in production; the app refuses to boot
# on the shipped placeholder values. Generate each with: openssl rand -hex 32
SNACKBASE_SECRET_KEY=<generate-strong-random-key>
SNACKBASE_TOKEN_SECRET=<generate-strong-random-key>
SNACKBASE_ENCRYPTION_KEY=<generate-strong-random-key>
...
| `SNACKBASE_SECRET_KEY`                  | placeholder — required in production         | JWT signing key                                |
| `SNACKBASE_TOKEN_SECRET`                | placeholder — required in production         | API key / SnackBase token signing key          |
| `SNACKBASE_ENCRYPTION_KEY`              | placeholder — required in production         | Encryption key for sensitive data at rest      |
...
<Note>Automated backup/restore commands will be added in Phase 5 (F5.10)</Note>
...
<Warning>
  Leaving `SNACKBASE_TRUSTED_PROXIES` at its loopback default behind a proxy collapses
  every visitor into one rate-limit bucket and one login-failure budget.
</Warning>
